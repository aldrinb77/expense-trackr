...
const allowedOrigins = new Set([
  "https://luminous-snickerdoodle-ec9624.netlify.app",
  "capacitor://localhost",
  "ionic://localhost",
  "http://localhost"
]);
...
app.use(
  cors({
    origin: (origin, callback) => {
      // Some WebView / server-to-server requests may have no Origin header
  ...
  try {
    ...
    for (const b of budgetList) {
      if (
        ...
        await upsertCategoryBudgetForUser(userId, b.category, b.monthlyBudget);
      }
    }
...
app.get("/api/saving-goal", authMiddleware, async (req, res) => {
  try {
    ...
    const goal = row && typeof row.monthlyGoal === "number" ? row.monthlyGoal : null;
    res.json({ goal });
  } catch (error) {
...
app.put("/api/saving-goal", authMiddleware, async (req, res) => {
  ...
  if (Number.isNaN(numGoal) || numGoal < 0) {
    return res.status(400).json({ error: "Goal must be a number 0 or greater." });
  }
...
app.post("/api/category-budgets", authMiddleware, async (req, res) => {
  ...
  try {
    const result = await upsertCategoryBudgetForUser(req.user.id, category, numBudget);
...
app.delete("/api/category-budgets/:category", authMiddleware, async (req, res) => {
  const encodedCategory = req.params.category;
  if (!encodedCategory) return res.status(400).json({ error: "Category is required." });

  const category = decodeURIComponent(encodedCategory);

  try {
    const deleted = await deleteCategoryBudgetForUser(req.user.id, category);
    if (!deleted) return res.status(404).json({ error: "Category budget not found" });
...
app.post("/api/transactions", authMiddleware, async (req, res) => {
  ...
  try {
    const created = await createTransaction(req.user.id, amount, type, category, note);
...
app.put("/api/transactions/:id", authMiddleware, async (req, res) => {
  ...
  try {
    const updated = await updateTransaction(req.user.id, id, amount, type, category, note);
...
// CSV export
app.get("/api/transactions/export/csv", authMiddleware, async (req, res) => {
  try {
    const rows = await getAllTransactionsForUser(req.user.id);

    const header = ["id", "amount", "type", "category", "createdAt", "note"];
